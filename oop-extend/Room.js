...
/*        if (drones.length == 0 && miners.length >= 1) {
        ...
        if (this.controller && this.controller.flag) {
            ...
            if (needs == 'Zealot') {
                ...
                var result = this.createCreep([WORK,WORK,WORK,WORK,WORK,CARRY,CARRY,CARRY,MOVE], undefined, { class: 'Zealot', home: this.name, flag: flag.name } );
        ...
        if (this.harvest_flags) {
            ...
            for (var i in this.harvest_flags) {
                ...
                if (needs == 'Fetcher') {
                    ...
                    var result = this.createCreep([WORK,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE], undefined, { class: 'Fetcher', home: this.name, mine: flag.pos.roomName, flag: flag.name } );
